<template>
  <div>
    ...
    <el-table
      ...
      <el-table-column label="操作" fixed="right" width="170" :formatter="isVisibleFun">
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="primary"
            @click="open(scope.$index, scope.row)">通过
          </el-button>
          <el-button
            size="mini"
            type="danger"
            @click="openMove(scope.$index, scope.row)">不通过
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    open(index, row) {
      this.showText = '此操作将不可恢复的进行项目结算, 是否继续?'
    ...
    openMove(index, row) {
      ...
      this.showText = '此操作将驳回本次项目结算申请, 是否继续?'
      this.$confirm(this.showText, '风险提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
